...
describe('<App />', () => {
  ...
  it('contains the Login component wrapped with WithLogging', () => {
    const wrapper = shallow(<App />);
    const bodySection = wrapper.find(BodySectionWithMarginBottom).at(0);
    const wrappedComponent = bodySection.props().children;
    expect(wrappedComponent.type.displayName).toBe('WithLogging(Login)');
  });
  ...
  describe('when isLoggedIn is true', () => {
    ...
    it('should not include the Login component', () => {
      const bodySection = wrapper.find(BodySectionWithMarginBottom).at(0);
      const wrappedComponent = bodySection.props().children;
      expect(wrappedComponent.type.displayName).not.toBe('WithLogging(Login)');
    });
